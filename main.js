...
function createHiraganaIcon(character) {
    ...
    context.font = `bold ${Math.floor(size * 0.7)}px sans-serif`;
...
async function initRecommendedSpots() {
    ...
    points.forEach((point, index) => {
        ...
        const hiraganaRegex = /^[\u3040-\u309F]$/; // 単一のひらがな文字にマッチする正規表現

        if (hiraganaRegex.test(iconValue)) {
            iconName = `hiragana-${iconValue}`;
            if (!map.hasImage(iconName)) {
                const canvas = createHiraganaIcon(iconValue);
                const context = canvas.getContext('2d');
                const imageData = context.getImageData(0, 0, canvas.width, canvas.height);
                map.addImage(iconName, imageData);
            }
        } else {
            iconName = 'mayor-icon'; // デフォルト
            if (iconValue) {
                switch (iconValue) {
                    case '市長':
                        iconName = 'mayor-icon';
